...
function Dashboard() {
  ...
  const userCurrency = (user?.moneda || 'USD').toUpperCase() as 'USD' | 'EUR' | 'ARS' | 'COP'
  
  const [currency, setCurrency] = useState<'USD' | 'EUR' | 'ARS' | 'COP'>(userCurrency)
  ...
  const userRealSavings = user?.saldoAhorrado !== undefined ? user?.saldoAhorrado : (userRealSaldo * 0.2)
  ...
  const getDisplayBalance = (baseAmount: number) => {
    if (currency === userCurrency) {
      return baseAmount.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    }
    ...
    return (baseAmount * rate).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
  }

  // Lista de últimos movimientos. Si el backend ya provee movimientos reales los renderiza, sino, usa los ilustrativos.
  const transactions = (user?.movimientos && user.movimientos.length > 0) ? user.movimientos : [
    {
      id: 1,
      ...
      icon: (
        <svg className="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 12H4m12 0l-4-4m4 4l-4 4" />
        </svg>
      ),
      bg: 'bg-red-50',
      color: 'text-red-600'
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h12M8 12l4-4m-4 4l4 4" />
        </svg>
      ),
      bg: 'bg-green-50',
  ...
  const savingsPercentage = Math.min(Math.round((userRealSavings / savingsGoal) * 100), 100)
  ...
  return (
    <div className="min-h-full pb-12">
      ...
      >
        <div>
          <p className="text-purple-100 text-sm font-medium">Bienvenido de nuevo,</p>
      ...
      <div className="px-8 -mt-16 grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100/50 flex flex-col justify-between min-h-[160px]">
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest">Saldo Disponible</span>
              </div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-gray-900">{currentSymbol}</span>
          ...
          <div className="mt-4 relative self-start">
            ...
            >
              <span>{currency === 'USD' ? '🇺🇸' : currency === 'EUR' ? '🇪🇺' : currency === 'ARS' ? '🇦🇷' : '🇨🇴'} {currency}</span>
              <svg className={`w-3 h-3 transition-transform duration-200 ${showCurrencyDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showCurrencyDropdown && (
              <>
                <div className="fixed inset-0 z-20" onClick={() => setShowCurrencyDropdown(false)}></div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100/50 flex flex-col justify-between min-h-[160px]">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest">Saldo Ahorrado</span>
            </div>
      ...
      <div className="px-8 mt-10">
        <h2 className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mb-4">Acciones Rápidas</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            >
              <div className={`w-12 h-12 rounded-full ${act.bg} ${act.iconColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}>
                {act.icon}
              </div>
              <span className="text-xs font-bold text-gray-700">{act.name}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="px-8 mt-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest">Últimos Movimientos</h2>
          <Link to={ROUTES.HISTORY} className="text-xs font-extrabold text-purple-600 hover:text-purple-700 hover:underline">
            Ver todos
          </Link>
        </div>
        ...
        <div className="space-y-3">
          {transactions.map((tx) => (
            <div 
              key={tx.id} 
              className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between transition-all hover:bg-gray-50/50"
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl ${tx.bg} flex items-center justify-center flex-shrink-0`}>
                  {tx.icon}
                </div>
                <div>
                  <h3 className="text-sm font-bold text-gray-800">{tx.title}</h3>
                  <p className="text-xs text-gray-400 font-medium mt-0.5">{tx.time}</p>
                </div>
              </div>

              <div className="text-right">
                <p className={`text-sm font-extrabold ${tx.color}`}>
                  {tx.amount}
                </p>
